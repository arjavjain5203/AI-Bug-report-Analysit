...
class RunResult:
    ...
    step_errors: List[str]  # For markdown/json
    console_logs: List[str]
    environment: Dict[str, Any]
    attachments: List[RunAttachment]
    bug_description: Optional[str] = None
    llm_bug_check: Optional[str] = None
    screenshots: Optional[List[str]] = None
    video_path: Optional[str] = None
    # For MongoDB
    step_errors_mongo: Optional[List[StepError]] = None

class BugReporter:
    def __init__(self, report_dir: str = None, mongo_db=None):
        self.report_dir = report_dir
        self.mongo_db = mongo_db

    def save_report(self, result: RunResult) -> Dict[str, str]:
        # Save to markdown/json as before
        if self.report_dir:
            ts = result.timestamp
        ...
        if self.mongo_db:
            ...
            # Convert step_errors for MongoDB
            if result.step_errors_mongo:
                report_data['step_errors'] = [asdict(e) for e in result.step_errors_mongo]
            insert_result = self.mongo_db.reports.insert_one(report_data)
            print(f"✅ Bug report saved to MongoDB with ID: {insert_result.inserted_id}")
            return {"mongo_id": str(insert_result.inserted_id)}
        return {}
...
# =========================
# ====== BUG CHECKER WITH LLM =====
# =========================

def check_for_bug_with_llm(result: RunResult, gemini_key: str) -> Tuple[bool, str]:
    """
    Analyzes the test result with an LLM to determine if a bug was found.
    
    Returns:
        A tuple: (is_bug_found, llm_explanation)
    """
    if not gemini_key:
        return False, "Gemini API key not provided, skipping bug validation."

    # Construct the prompt payload
    expected_behavior = "All steps should complete successfully without any console errors or step failures."
    actual_behavior = "The test run encountered one or more issues."
    
    # Summarize step errors if any
    if result.step_errors:
        actual_behavior += " The following step(s) failed: "
        actual_behavior += " ".join([f"Step {e.step_index}: {e.error_message}" for e in result.step_errors])
    
    # Summarize console logs if any
    if result.console_logs:
        actual_behavior += f" The browser console contained {len(result.console_logs)} errors or warnings."
        
    full_logs = "\n".join(result.console_logs)

    # The prompt to the LLM
    prompt = f"""
Based on the expected behavior and actual behavior, determine if this is a bug.

If it is a bug, return "Bug Found" and explain why.
If not, return "No Bug".
...
"""
    try:
        ...
        # Check if the LLM's response indicates a bug
        is_bug = text.lower().startswith("bug found")
        return is_bug, text

    except Exception as e:
        print(f"Error calling Gemini API: {e}", file=sys.stderr)
        return False, f"Failed to check for bug with LLM. Error: {e}"


# =========================
# ====== MAIN ENTRYPOINT ======
# =========================
# ... (rest of the main entrypoint code remains the same) ...
...
async def main(args: argparse.Namespace):
    ...
    # 🌟 NEW: Check for bug with LLM
    is_bug_found, llm_explanation = check_f
# For production, use environment variables for security (e.g., os.environ.get("MONGO_URI"))or_bug_with_llm(result, gemini_key)
    result.llm_bug_check = llm_explanation
    result.bug_description = args.bug

    # Save report
    reporter = BugReporter(REPORT_DIR if not mongo_db else None, mongo_db)
    ...
    print(f"Console errors: {len(result.console_logs)}")
    print(f"LLM Bug Check: {'✅ BUG FOUND' if is_bug_found else '❌ No Bug Found'}")
    print(f"LLM Explanation: {llm_explanation}")
    if mongo_db:
        print(f"Report saved to MongoDB with ID: {paths.get('mongo_id')}")
    else:
        print(f"Report saved at: {paths.get('md')}")
        print(f"Full JSON data at: {paths.get('json')}")
...
def parse_args():
    ...
    ap.add_argument("--mongo", action="store_true", help="Save report to MongoDB")
...
async def main():
    ...
    # MongoDB setup
    mongo_db = None
    if args.mongo and MONGO_AVAILABLE:
        client = MongoClient("mongodb://localhost:27017/")
        mongo_db = client.bugfiner_db
    ...
    # 🌟 NEW: Check for bug with LLM
    is_bug_found, llm_explanation = check_for_bug_with_llm(result, gemini_key)
    result.llm_bug_check = llm_explanation
    result.bug_description = args.bug
